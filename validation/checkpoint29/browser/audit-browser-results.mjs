...
check("SUMMARY_GATE", summary.browser_validation === "PASS" && summary.gate === "PASS_W4_BROWSER" && summary.wave_gate === "CYCLE4_COMPONENT_ABSENT_DOM_STATE", `${summary.browser_validation}/${summary.gate}/${summary.wave_gate}`);
check("WAVE_IDENTITY", summary.checkpoint === 29 && summary.wave?.id === "W4" && summary.totals?.planned === 2 && summary.totals?.passed === 2 && summary.totals?.browser_checks === 38, `CP${summary.checkpoint}; ${summary.wave?.id}; ${summary.totals?.passed}/${summary.totals?.planned}; checks=${summary.totals?.browser_checks}`);
...
check("ORACLE_IDENTITY", oracle.checkpoint === 29 && oracle.wave === "W4" && caseIds.length === 2 && oracle.source.includes("CP23"), `CP${oracle.checkpoint}; ${oracle.wave}; ${caseIds.length} casos; ${oracle.source}`);
...
check("CHECKSUMS", mismatches.length === 0 && sums.length === 47, mismatches.length ? mismatches.join(", ") : `${sums.length}/47`);

const status = checks.every(({status: itemStatus}) => itemStatus === "PASS") ? "PASS" : "FAIL";
const result = {
  schema_version: "oe-irp-f2-browser-audit-0.5",
  checkpoint: 29,
  wave: "W4",
  ...
  status,
  checks,
  gate: status === "PASS" ? "PASS_W4_BROWSER" : "FAIL",
  next_gate: status === "PASS" ? "ENABLE_F2_CONSOLIDATED_CLOSURE" : "REMEDIATE_AND_REPEAT_W4",
  limits: [
    "El componente reservado a Ciclo 4 permanece ausente del DOM y del estado de C2/C3.",
    "Ciclo 4 no se habilita; main, Pages y baselines permanecen sin cambios."
  ]
};
