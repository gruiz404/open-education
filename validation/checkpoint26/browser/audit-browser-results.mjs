...
check("SUMMARY_GATE", summary.browser_validation === "PASS" && summary.gate === "PASS_W1_BROWSER" && summary.wave_gate === "STATE_HISTORY_UNDO_CONFORM", `${summary.browser_validation}/${summary.gate}/${summary.wave_gate}`);
check("WAVE_IDENTITY", summary.checkpoint === 26 && summary.wave?.id === "W1" && summary.totals?.planned === 8 && summary.totals?.passed === 8, `CP${summary.checkpoint}; ${summary.wave?.id}; ${summary.totals?.passed}/${summary.totals?.planned}`);
...
check("CHECKSUMS", mismatches.length === 0 && sums.length === 185, mismatches.length ? mismatches.join(", ") : `${sums.length}/185`);

const status = checks.every(({status: itemStatus}) => itemStatus === "PASS") ? "PASS" : "FAIL";
const result = {
  schema_version: "oe-irp-f2-browser-audit-0.2",
  checkpoint: 26,
  wave: "W1",
  ...
  status,
  checks,
  gate: status === "PASS" ? "PASS_W1_BROWSER" : "FAIL",
  next_gate: status === "PASS" ? "ENABLE_F2_W2" : "REMEDIATE_AND_REPEAT_W1",
  limits: [
    "No certifica comprensión, logro cognitivo ni validez del sistema publicado.",
