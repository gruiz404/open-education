...
check("SUMMARY_GATE", summary.browser_validation === "PASS" && summary.gate === "PASS_W3_BROWSER" && summary.wave_gate === "ALLOWLIST_EXACT_SCOPE_CONFORM", `${summary.browser_validation}/${summary.gate}/${summary.wave_gate}`);
check("WAVE_IDENTITY", summary.checkpoint === 28 && summary.wave?.id === "W3" && summary.totals?.planned === 3 && summary.totals?.passed === 3 && summary.totals?.browser_checks === 57, `CP${summary.checkpoint}; ${summary.wave?.id}; ${summary.totals?.passed}/${summary.totals?.planned}; checks=${summary.totals?.browser_checks}`);
...
check("ORACLE_IDENTITY", oracle.checkpoint === 28 && oracle.wave === "W3" && caseIds.length === 3 && oracle.source.includes("CP23"), `CP${oracle.checkpoint}; ${oracle.wave}; ${caseIds.length} casos; ${oracle.source}`);
...
check("CHECKSUMS", mismatches.length === 0 && sums.length === 70, mismatches.length ? mismatches.join(", ") : `${sums.length}/70`);

const status = checks.every(({status: itemStatus}) => itemStatus === "PASS") ? "PASS" : "FAIL";
const result = {
  schema_version: "oe-irp-f2-browser-audit-0.4", checkpoint: 28, wave: "W3",
  auditor: "IRP-F2-BROWSER-EXTERNAL-AUDITOR", auditor_version: "0.4.0", status, checks,
  gate: status === "PASS" ? "PASS_W3_BROWSER" : "FAIL",
  next_gate: status === "PASS" ? "ENABLE_F2_W4" : "REMEDIATE_AND_REPEAT_W3",
  limits: [
    "No interpreta relevancia, recomienda acciones, confirma hipótesis ni certifica comprensión o logro.",
